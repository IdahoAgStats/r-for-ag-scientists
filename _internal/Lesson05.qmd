...
If a particularily large file is loaded into R, using `View()` may be very slow. In that case, you can use `str()` to look a data frame structure:
...
Let's image that we want to assign the third value in the second column as `NA`. First, we index the that position, then we assign a value to it:
...
A very mile-high view of the matrix is given here because you may encounter this, but it's less commonly used data structure in R. 
...
A matrix lacks some of the attributes and functionality that are possible for data.frames. Colnames can be given, but they cannot be used to index columns (i.e. `my_matrix$col` will throw an error). 
...
Coercion is also possible. If you find yourself working with matrices, you can convert a data.frame to a matrix. Or a function may return a matrix that you need converted back to a data frame:
```{r, eval=FALSE}
as.data.frame()
as.matrix()
as.list()
