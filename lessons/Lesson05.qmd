...
The `rownames` attribute can be set, but if it is not, it automatically created within R from 1 to the total number of rows. Row names are a tricky attribute than many packages in R do not support. A function may toss of your row names without any warning! 
...
```
*(This graphic is an overlay of green over blue, creating a dark teal color. The green represents rows indexed, the blue is columns indexed and the teal is the intersection between those two. If a color is missing, it is presumed to be under the teal overlay.)* 
