...
-   be able to create a sequence of numbers in R using any starting
    value and any ending value
:::
...
`TRUE` `FALSE` \| logical \| \| `NA` \| missing value \| \| `NaN` \| not
a number/undefined \| \| `NULL` \| no value/undefined \| \| `Inf`,
`-Inf` \| infinity \| \| `for` `in` \| for loops \| \| `if` `else`
`while` `break` `next` `repeat` \| control flow \| \| `NA_integer_`
`NA_real_` `NA_complex_` `NA_character_` \| missing data by class
