...
4.  Change an Item To change the value of a specific item, refer to the
    index number:
...
The most common object types used includes numeric, logical,and
character.

-   Numeric Previously, we created an object in R that was a collection
    or sequence of numbers. These numbers are technically integers
...
Here, x8, x9, x10 are numeric and x11 is character. These can coerced
...
Notes that is a single value is supplied for a column, it will be
repeated for the entire column.
...
Like vectors, data frames can be indexed by position, except now we have
two dimensions to consider. You can extract individual elements in a
data frame by references the row and column position,
...
sequence numbers from zero to -4.
...
Let's image that we want to assign the third value in the second column
