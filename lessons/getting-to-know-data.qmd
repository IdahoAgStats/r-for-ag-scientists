...
This code `read.csv()` reads a CSV file named "caribbean_maize.csv" and assigns it to an object named "data1".

The `View()` will open the data set in new RStudio video where you can look up at the rows and columns. The second line of code uses the "head" function to display the first 5 rows of the "data1" data frame. 
...
Barplots are also handy. There is no column `data1` is suited for a barplot, but we can make data that will work well for a barplot:

```{r}
data1$yield_categories <- cut(data1$yield, 5) # break yield into 5 groups, each equidistant
