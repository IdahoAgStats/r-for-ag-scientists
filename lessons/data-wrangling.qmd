...
-   be able to use `separate()` to split up single variables into multiple variables
:::
...
mutate1 <- mutate(mutate1, 
                     row_position = 1:n())

## use existing column with different name
mutate1 <- mutate(mutate1, 
                    trial_id = trial)
...
## change class of existing variable
mutate1 <- mutate(mutate1, 
                    rep_factor = as.factor(rep))

head(mutate1)
```

All the above performed tasks can we done in one go. 
```{r}
mutate2 <- mutate(variety_trials, 
                    dataset = "example",
                    row_position = 1:n(),
                    trial_id = trial,
                    yield_new = yield+50,
...
-   **trial_id**, renamed the **trial** column.
-   **yield_new** which is the sum of the value for yield plus 50. 
-   **yield_protein** the addition of two variables in the data set (this is also vectorized, calculating this for each row)
-   **rep_factor** which is a new column to represent the **replicate**, but the class of a new variable is 'factor' instead of 'numeric'.
...
1. Import "weather.csv" with the `readr::read_csv()`. 
1.  Create a new variable named T_avg as an average of 'tmin_F' and 'tmax_F' using a `mutate()` function and name this data frame as 'weather1'.
1. Select "name" column from the weather1 data set and rename it as "location".
1. In weather1 data set, select the first five columns and reduce that data set to unique rows (look into using `distinct()` for extracting the unique observations). 
1. Using weather1 data set, create a new variable as combination of station and name variables separated by "_" (e.g. station_name) using the unite() function. 
