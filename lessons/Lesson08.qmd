...
This lesson is focused on exporting tabular data. It is a very short lesson because exporting data is quite similar to importing data. Like in data import, exporting data invovles opening a connection between R and file system, writing the data to file and closing the connection.
...
Setting the `row.names` argument to `FALSE` ensures that a column of numbers without a header is not included in the file (which is likely to cause import errors in the future). \### `write.table()`
...
::: callout-note
If you are working with a specialized file type that has dedicated libraries for importing them into R and manipulating those objects, those dedicated libraries likely have export functions for that file type. For spatial object, the packge **sf** can import, alter, and export shapefiles.
:::
...
It is very important that you check your output file (especially as a newer R programmer!) to make sure everything is as you expect. We have all accidentally output "myfile" instead of "myfile.csv", and this can be highly inconveniencing! You can do by manually opening the file or by importing back into R; either should tell you if the file export went as expected or not.
:::
