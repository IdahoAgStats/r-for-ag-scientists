...
::: callout-note
`summarise()` only returns a single value back for each group. If you want that single value applied to each observation, use `mutate()` within a `group_by()` statement.
:::
...
# arrange those 50 data points into a data frame of 5 columns, each column reflecting 10 observations
...
1. Import "weather_data.csv", select 'station, 'tmin_F' and 'tmax_F' using the `select()` function. Calculate the sum, mean, and median for 'tmin_F' and 'tmax_F' using `rowwise()` function.
