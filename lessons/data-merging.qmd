...
Before working on merging and binding data sets, let's create two subsets from the "variety_trial" data set. Here, we are using the `filter()` and `select()` functions to create two smaller data sets. We have created two data sets, where each contain a single trial and selected matching columns.
...
If you have two data sets of different observations (the keys do not match) but similar or identical column headers, these rows can be stacked together using a row bind.
...
We have manufactured a version of this with our data sets by filtering to a single trial and selecting a few columns. This is a silly toy example, but most of the time you will not be handed these data sets that are already merged. You will be given two or more data sets that need to be combined. Perhaps these are field experiments from different years or lab results from two different runs.

We will start by looking at the two example data sets we created above, `trial_1` and `trial_2`.
...
Here is the table with summary of **dplyr** & **tidyr** functions we have used in this lesson and previous lessons:

| Function       | Package | Description                                |
|----------------|---------|--------------------------------------------|
| filter()       | dplyr   | Keep rows that meet a condition            |
| select()       | dplyr   | Keep or reorder columns                    |
| mutate()       | dplyr   | Keep or reorder columns                    |
| arrange()      | dplyr   | Sort rows                                  |
| group_by()     | dplyr   | Group data for summarizing                 |
| rename()       | dplyr   | Rename columns                             |
| case_when()    | dplyr   | Create conditional new columns             |
| pivot_longer() | tidyr   | Convert wide → long format                 |
| pivot_wider()  | tidyr   | Convert long → wide format                 |
| drop_na()      | tidyr   | Remove rows containing missing values (NA) |
| separate()     | tidyr   | Split a column into multiple columns       |
| unite()        | tidyr   | Combine multiple columns into one          |

: Summary of functions in dplyr and tidyr packages {#tbl-opt_pkg}

If you would like to explore more functions in these packages, please refer to
[dplyr functions](https://dplyr.tidyverse.org/reference/index.html) and [tidyr functions](https://tidyr.tidyverse.org/reference/index.html) documentation.
...
2.  Do an inner join between "genotypic_data_rotated.csv" and "trial_data.csv" using variety names.

3.  Do a semi-join of "genotypic_data_rotated.csv" with "trial_data.csv" and do the reverse. How does this compare with the inner join from the previous problem?

4.  Do an anti-join between "genotypic_data_rotated.csv" and "trial_data.csv".

5.  Join together all common observations between the 3 files (your choice on join).
:::
