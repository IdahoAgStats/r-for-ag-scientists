...
This is the last lesson with a major focus on 'base R', that is the set of functions that come automatically loaded when you install and open R.
...
Here are some other [R manuals](https://rstudio.github.io/r-manuals/). 

### Libraries

The R community keeps R humming and current by writing packages that extend R's functionality. This is both awesome (the latest greatest tool is now enabled!) and bad (quality of implementation is not guaranteed and these packages are often not maintained over the long haul).
