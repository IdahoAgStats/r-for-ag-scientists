...
1. Using hay_idaho, create a line and point plot for number of acres over years. Assign a color and shape to line and points of your choice.

1. Import Apple.csv, create a box-plot of 'yield' for 'stock' variable. Add points over the boxplots.
...
(@) Import the "weather_data.csv" from the data folder, group by 'station' and calculate mean and standard deviation for 'tmin_F'. Create a point plot with error bars for 'tmin_F' (x-axis) and 'station'. Specify the shape, size, and color of the points (your choice).
...
1. Using hay_pnw, ceate a point scatter plot for yield over years and add color & shape effect for the state factor. 
  ...
  * Assign this graph to an object named 'plot1'.
